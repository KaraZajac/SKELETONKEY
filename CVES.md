...
**Counts:** 44 modules total covering 39 CVEs; **28 of 39 CVEs
...
> `ptrace_pidfd` / `sudo_host` / `cifswitch` / `nft_catchall` / `bad_epoll` are blocked by their target environment (VMware-only,
> kernel < 4.4, mainline panic, kmod not autoloaded, t64-transition
> libs) or are brand-new this cycle, not by missing code (`bad_epoll` is
> a reconstructed epoll race trigger — deliberately under-driven and not
> VM-verified). See
